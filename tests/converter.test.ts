...
describe("convertClaudeToOpenCode", () => {
  test("from-command mode: map allowedTools to global permission block", async () => {
    ...
    expect(
      bundle.commandFiles.find((f) => f.name === "workflows:review")
    ).toBeDefined();
    expect(
      bundle.commandFiles.find((f) => f.name === "plan_review")
    ).toBeDefined();
    ...
    const agentFile = bundle.agents.find(
  ...
  test("normalizes models and infers temperature", async () => {
    ...
    const securityAgent = bundle.agents.find(
      (agent) => agent.name === "security-sentinel"
    );
    ...
    const modelCommand = bundle.commandFiles.find(
      (f) => f.name === "workflows:work"
    );
  ...
  test("converts hooks into plugin file", async () => {
    ...
    const hookFile = bundle.plugins.find(
      (file) => file.name === "converted-hooks.ts"
    );
    ...
    expect(hookFile!.content).toContain(
      "// Agent hook for Write|Edit: security-sentinel"
    );
  });
  ...
  test("excludes commands with disable-model-invocation from commandFiles", async () => {
    ...
    expect(
      bundle.commandFiles.find((f) => f.name === "deploy-docs")
    ).toBeUndefined();
      ...
      commands: [
        {
          ...
          body: `Read \`agentic-engineering.local.md\` in the project root.

If no settings file exists, auto-detect project type.

Run \`/agentic-engineering-setup\` to create a settings file.`,
    ...
    expect(commandFile!.content).toContain("agentic-engineering.local.md");
  });
  ...
  test("rewrites .claude/ paths in agent bodies", () => {
      ...
      agents: [
        {
          ...
          body: "Read `agentic-engineering.local.md` for config.",
    ...
    expect(agentFile).toBeDefined();
    // Tool-agnostic path in project root — no rewriting needed
    expect(agentFile!.content).toContain("agentic-engineering.local.md");
  });
