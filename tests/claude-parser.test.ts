...
const invalidCommandPathRoot = path.join(
  import.meta.dir,
  "fixtures",
  "invalid-command-path"
);
const invalidHooksPathRoot = path.join(
  import.meta.dir,
  "fixtures",
  "invalid-hooks-path"
);
const invalidMcpPathRoot = path.join(
  import.meta.dir,
  "fixtures",
  "invalid-mcp-path"
);
...
describe("loadClaudePlugin", () => {
  test("loads manifest, agents, commands, skills, hooks", async () => {
    ...
    const researchAgent = plugin.agents.find(
      (agent) => agent.name === "repo-research-analyst"
    );
    expect(researchAgent?.capabilities).toEqual([
      "Capability A",
      "Capability B",
    ]);

    const reviewCommand = plugin.commands.find(
      (command) => command.name === "workflows:review"
    );
    ...
    const planReview = plugin.commands.find(
      (command) => command.name === "plan_review"
    );
    expect(planReview?.allowedTools).toEqual(["Read", "Edit"]);

    const skillCommand = plugin.commands.find(
      (command) => command.name === "create-agent-skill"
    );
    expect(skillCommand?.allowedTools).toEqual(["Skill(create-agent-skills)"]);

    const modelCommand = plugin.commands.find(
      (command) => command.name === "workflows:work"
    );
    expect(modelCommand?.allowedTools).toEqual(["WebFetch"]);

    const patternCommand = plugin.commands.find(
      (command) => command.name === "report-bug"
    );
    expect(patternCommand?.allowedTools).toEqual(["Read(.env)", "Bash(git:*)"]);

    const planCommand = plugin.commands.find(
      (command) => command.name === "workflows:plan"
    );
    expect(planCommand?.allowedTools).toEqual([
      "Question",
      "TodoWrite",
      "TodoRead",
    ]);

    expect(plugin.mcpServers?.context7?.url).toBe(
  ...
  test("parses disable-model-invocation from commands", async () => {
    ...
    const disabledCommand = plugin.commands.find(
      (command) => command.name === "deploy-docs"
    );
    expect(disabledCommand).toBeDefined();
    expect(disabledCommand?.disableModelInvocation).toBe(true);

    const normalCommand = plugin.commands.find(
      (command) => command.name === "workflows:review"
    );
  ...
  test("merges default and custom component paths", async () => {
    ...
    expect(plugin.agents.map((agent) => agent.name).sort()).toEqual([
      "custom-agent",
      "default-agent",
    ]);
    expect(plugin.commands.map((command) => command.name).sort()).toEqual([
      "custom-command",
      "default-command",
    ]);
    expect(plugin.skills.map((skill) => skill.name).sort()).toEqual([
      "custom-skill",
      "default-skill",
    ]);
    expect(plugin.hooks?.hooks.PreToolUse?.[0]?.hooks[0]?.command).toBe(
      "echo default"
    );
    expect(plugin.hooks?.hooks.PostToolUse?.[0]?.hooks[0]?.command).toBe(
      "echo custom"
    );
  });
