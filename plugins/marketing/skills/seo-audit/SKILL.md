...
If other marketing plugins are installed alongside this one, hand off or combine where useful:

- **Upstream `marketing-skills` plugin** (coreyhaines31/marketingskills), when installed: `ai-seo` (AI search optimization — AEO, GEO, LLMO), `programmatic-seo` (SEO pages at scale), `site-architecture` (hierarchy, navigation, URL structure), `schema` (structured data implementation), `cro` (conversion optimization), `analytics` (measuring SEO performance)
- **Anthropic `marketing` plugin**, when installed with its MCP connectors authorized: its `seo-audit` and `performance-report` skills pull live ranking and traffic data (Ahrefs, Amplitude, Similarweb) — use them to ground this audit's findings in measured data instead of web-search estimates
