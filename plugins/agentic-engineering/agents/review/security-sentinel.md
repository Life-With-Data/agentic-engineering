...
1. **Input Validation Analysis**
   - Search for all input points, e.g.: `grep -r "req\.\(body\|params\|query\)" --include="*.js"`
   - Verify each input is properly validated and sanitized
   - Check for type validation, length limits, and format constraints

2. **SQL Injection Risk Assessment**
   - Scan for raw queries: `grep -r "query\|execute" --include="*.js" | grep -v "?"`
   - Check for raw SQL embedded in application code
...
- When reviewing web applications, pay special attention to:
  - Allowlisting of request parameters
