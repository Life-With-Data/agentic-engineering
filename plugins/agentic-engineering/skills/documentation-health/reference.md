...
*Granting the agent its tools.* CI's non-interactive mode is deny-by-default: with no allowlist the agent can't run the scanner **or** post the review — both are denied and the tier silently degrades to structured-output-only. Grant tools two ways, and mind *where* each goes. Scanner + skill go in the **`settings` input** as `permissions.allow` (`Bash(python3:*)`, `Skill`, `Read`, `Glob`, `Grep`): it's parsed as JSON (no shell-quote mangling) and the action writes it to user-scope `~/.claude/settings.json`, whose allow rules apply headless — project-scope rules would need a workspace-trust dialog that `-p` never shows. The single review tool, `mcp__github__create_and_submit_pull_request_review`, goes in **`claude_args` as `--allowedTools`**: the action injects its GitHub MCP server only when an `mcp__github__*` tool appears there, and `--allowedTools` is *additive* with the settings allowlist (it auto-approves; it doesn't replace it). Keep `Bash(...)` rules OUT of `claude_args` — `shell-quote` mangles their parens/colons, the same failure class that ate the `--json-schema` quotes. That review tool posts one **body-only** formal review, which (unlike the diff-anchored inline-comment server, or the sticky-comment server that needs a tracking comment agent mode never creates) can reference whole-repo findings that aren't on the PR's diff.
