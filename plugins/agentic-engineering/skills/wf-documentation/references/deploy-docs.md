...
```bash
# Count components
echo "Agents: $(ls plugins/agentic-engineering/agents/*.md | wc -l)"
echo "Commands: $(ls plugins/agentic-engineering/commands/*.md | wc -l)"
echo "Skills: $(ls -d plugins/agentic-engineering/skills/*/ 2>/dev/null | wc -l)"

# Validate JSON
cat .claude-plugin/marketplace.json | jq . > /dev/null && echo "✓ marketplace.json valid"
cat plugins/agentic-engineering/.claude-plugin/plugin.json | jq . > /dev/null && echo "✓ plugin.json valid"

# Check all HTML files exist
for page in index agents commands skills mcp-servers changelog getting-started; do
  if [ -f "plugins/agentic-engineering/docs/pages/${page}.html" ] || [ -f "plugins/agentic-engineering/docs/${page}.html" ]; then
...
```bash
git status --porcelain plugins/agentic-engineering/docs/
```
...
on:
  push:
    branches: [main]
    paths:
      - 'plugins/agentic-engineering/docs/**'
  workflow_dispatch:
  ...
  deploy:
    ...
    steps:
      ...
      - uses: actions/upload-pages-artifact@v3
        with:
          path: 'plugins/agentic-engineering/docs'
