...
description: Instrument code so production behavior is visible and diagnosable. Use when adding logging, metrics, tracing, or alerting. Use when shipping any feature that runs in production and evidence it works is required. Use when a production issue is reported but the available telemetry can't explain what happened.
---
...
Code that can't be observed is code that can't be operated. Observability is the ability to answer "what is the system doing, and why?" from the outside, using the telemetry the code emits. Instrumentation is not a post-launch add-on — write it alongside the feature, the same way tests are written. A feature that ships without telemetry turns the first user-reported bug into archaeology instead of a query.
...
If the questions can't be named, the feature isn't ready to instrument — the result is logging everything and learning nothing.
...
**Log levels — apply them consistently:**
...
**Correlation IDs are mandatory.** Generate (or accept) a request ID at the system boundary and attach it to every log line, span, and outbound call. Without it, there is no way to reconstruct a single request from interleaved logs:
...
**Never log secrets, tokens, passwords, or full PII.** Telemetry pipelines are a classic data-leak path — the same secret-handling discipline a security-and-hardening review enforces. Allowlist fields explicitly; never log whole request or response bodies. The `security-sentinel` agent flags leaked secrets at review time.
...
Cause-based alerts fire when nothing is wrong and miss failures that weren't predicted. Symptom-based alerts fire exactly when users are hurt, regardless of the cause.
...
|---|---|
| "I'll add logging after it works" | "After" becomes "after the first incident", which is the most expensive moment to discover the system is blind. Instrument as you build. |
