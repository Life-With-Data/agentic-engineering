...
Returns JSON: `{"flags": [...], "ok": true}`. Each entry has `key`, `kind` (`boolean`/`enum`/`list`/`duration`/`identity`), `choices` (enum values; empty otherwise), `default`, `effective`, `set`, `valid`, `source` (`local`/`committed`/`default`), `toggleable`, `file`, `owner`, `description`, `plugin`.
