...
2. **Out-of-band PRs from a trusted author** — the reconciler's **rule 5** (`a trusted open closing PR on an in_progress item → in_review`) advances them at the next command entry. No token, no Actions minutes, no extra file.

A PR from an **untrusted** author is deliberately **not** auto-advanced (the security model flags it for human review rather than trusting it to drive state) — which is why a blanket "any linked PR → in_review" Actions job was the wrong tool. Trust here is the same provenance rule the issue author gets: OWNER/MEMBER/COLLABORATOR, or a GitHub App. Note this is scoped by **provenance, not by assignment** — an earlier `author in assignees` filter silently disabled rules 3 and 5 for every bot-authored PR and every unassigned issue.
