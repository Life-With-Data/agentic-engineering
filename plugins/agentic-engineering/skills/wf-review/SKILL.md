...
7. Produce a clear ready/not-ready decision and return it with its findings to the caller (`wf-orchestrate` in the standard pipeline); a not-ready verdict names the blocking findings for the development stage, and a ready verdict records the reviewed head SHA so downstream stages can check freshness.
