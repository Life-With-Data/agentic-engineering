---
name: analyze-source
description: Evaluate any external resource (an X post, a blog, a GitHub repo, a marketplace, an installable tool) for its value to this repo, and return exactly one verdict — author locally, track as an upstream source, spin up a new domain plugin, reference/install-alongside, or skip. Use when someone shares a resource and asks whether and how it should influence this plugin.
argument-hint: "<url or owner/repo — X post, blog, GitHub repo, marketplace, tool>"
disable-model-invocation: true
allowed-tools: Read, Grep, Glob, Task, WebFetch, WebSearch, Bash(gh api *), Bash(gh repo view *)
---
...
- **Non-interactive by contract.** Never prompt the user mid-run. Record ambiguities in the
  report and continue to a verdict. This command is designed to be delegated: a caller can
  hand a background agent the single instruction "run `/analyze-source <url>`" and expect a
  complete report back with no round-trips.
...
2. **Track as an upstream source** *(the intake exit — repos worth cherry-picking from
   ...
   into the registry; `/upstream-scan` takes over the recurring scan from there. Template:
...
- Fully delegable: a background agent handed `/analyze-source <url>` returns this report with no
