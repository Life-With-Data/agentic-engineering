...
description: Apply a systematic root-cause debugging methodology when something breaks — stop the line, then reproduce, localize, reduce, fix the root cause, guard, and verify. Use when a test fails, a build breaks, behavior diverges from expectations, or an unexpected error surfaces and the cause is not yet understood. This is the broader triage methodology; for the concrete reproduce-and-file workflow, use the reproduce-bug and report-bug commands and the bug-reproduction-validator agent.
---
