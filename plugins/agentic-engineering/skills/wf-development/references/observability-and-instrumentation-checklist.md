...
- [ ] No secrets, tokens, passwords, or unredacted PII in any log line (hard rule; audited by the `security-sentinel` agent at review time)
...
The launch-day monitoring sequence and rollback triggers live in the launch runbook, not here — this gate covers only the instrumentation that must already exist before that sequence can run.
