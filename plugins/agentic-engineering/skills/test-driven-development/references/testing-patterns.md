...
Quick reference for common testing patterns across the stack. Use alongside the [test-driven-development](../SKILL.md) skill — the skill covers *when* and *why* (RED-GREEN-REFACTOR, the Prove-It pattern, the double-preference order); this file collects the concrete *how*.
...
> The examples use a TypeScript/Jest-style stack for concreteness. Translate the syntax to the project's actual framework (pytest, RSpec, Go's `testing`, etc.) — the patterns are language-agnostic.
...
Prefer asserting on state (the returned value, the persisted record) over interactions (which methods were called). State-based assertions survive refactors; interaction-based ones break the moment the internals change.
...
For agent-driven browser verification without a Playwright harness, the `agent-browser` skill and `/test-browser` command drive a real headless browser via Bash — useful for reproducing and confirming UI bugs mid-change.
