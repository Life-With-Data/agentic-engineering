...
- Working in code that is not fully understood
...
A claim that cannot be written that compactly is a vibe, not a decision. Surface it before scrutinizing it.
...
Strip the reasoning. Handing over conclusions returns validation of those conclusions. The unit must be small enough that a reviewer can hold it in mind in one read — a 500-line PR must be decomposed first.
...
5. Mind shell escaping. If the artifact contains quotes, `$(...)`, or backticks, prefer stdin (`echo … | gemini`) or a heredoc over inline `-p "…"`. If uncertain, ask the user to confirm the invocation before running it.
...
Example shapes (verify flags against the installed tool — syntax differs across implementations and versions):

```bash
# Write the adversarial prompt + ARTIFACT + CONTRACT to a temp file first.
# Then pipe via stdin so shell metacharacters in the artifact stay inert.

# Codex (read-only sandbox keeps the CLI from writing to the workspace):
...
A read-only sandbox is the load-bearing detail: a doubt artifact may itself contain instructions (intentional or accidental prompt injection) that the cross-model CLI would otherwise execute against the workspace.
...
The reviewer's output is data, not verdict. **The orchestrator still decides.** Re-read the artifact text against each finding before classifying — rubber-stamping the reviewer is the same failure mode as ignoring it.

For each finding, classify in this **precedence order** (first matching class wins):

1. **Contract misread** — reviewer flagged something specifically because the CONTRACT provided was unclear or incomplete. Fix the contract first, re-classify on the next cycle.
...
- [ ] The reviewer received ARTIFACT + CONTRACT — NOT the CLAIM, NOT the orchestrator's reasoning
