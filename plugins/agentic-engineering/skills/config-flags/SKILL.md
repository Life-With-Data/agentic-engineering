---
name: config-flags
description: Browse and toggle every opt-in configuration flag the plugin offers for this repo — current value vs. default, with a guided flip. Named config-flags (not config) to avoid colliding with Claude Code's built-in /config.
argument-hint: "[key] [value] — non-interactive set, or omit for the interactive browser"
allowed-tools: Read, Bash(python3 *), Bash(git *)
---

# Config Flags

Discoverability front door for the plugin's opt-in, per-repo configuration flags (`issue_tracker`, `nudge_todowrite`, etc.). Complements [`lifecycle-doctor`](lifecycle-doctor.md): doctor reports the health of what's *currently* configured; this command shows *everything that could be* configured and lets you change it. Unlike doctor, it is not silent by design — every flag is always listed, regardless of state.
...
- Rows with `kind: list` (e.g. `review_agents`) are not editable here yet — if the user's intent seems to be changing one of these, point them at `/setup` instead of offering a broken flip.
