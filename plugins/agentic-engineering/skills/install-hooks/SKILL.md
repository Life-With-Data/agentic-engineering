---
name: install-hooks
description: Wire the plugin's portable safety hooks (block --no-verify, prevent main commits, block Slack webhook leaks, block prisma db push) into the current coding agent. Use after installing skills via the skills CLI (npx skills add), which installs skills only and does not carry hooks. Triggers on "install hooks", "set up safety hooks", "enable the guardrails".
...
Determine the absolute path of the directory containing this SKILL.md; the
scripts live in `scripts/` beside it. Where this skill lives depends on how it
was installed (project `.claude/skills/` or `.agents/skills/`, global
`~/.claude/skills/`, …), so resolve it from the actual file location rather
than assuming. Confirm `python3` is on PATH (the scripts are stdlib-only).
