...
2. Branch on `verdict` — exactly these outcomes, nothing else:
   ...
   - **`already_done`** (groomed past brainstorming — the item is at `brainstormed` **or any later stage**) → the gate has already advanced beyond this route's scope. Announce the current stage and follow the gate's route (for example, `route_to_plan` selects the `wf-grooming` planning route; a `ready_for_work`-or-later item selects `wf-development`, while a `planned` item is groomed but not yet approved and awaits a human's `ready_for_work` stamp). Then STOP — **never re-groom and never re-stamp**. Brainstorm never regresses a later stage back to `brainstormed`.
