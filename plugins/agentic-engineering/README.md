...
`wf-auto` is the unattended front door onto the same pipeline: it selects the ticket when none is named and suppresses every optional check-in. It grants no authority the orchestrator lacks — an unapproved item still ends the run.
