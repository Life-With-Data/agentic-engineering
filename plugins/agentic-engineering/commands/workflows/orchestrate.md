...
| Work: open the PR | **AUTO** | `/workflows:work` Phase 4 opens the PR with `Closes #N` and sets Status=`in_review` (the issue is **not** closed at PR creation — the built-in "Item closed" automation owns `shipped` at merge, via `--set-status`). Outward-facing, but it's the expected terminal of the work stage in a solo/small-team flow. |
