...
- **3 more operational hooks adapted from agent-leverage's toolchain, rounding out the safety-net set alongside `block-no-verify` and `prevent-main-commit`.**
  - **`block-beads-jsonl-stage` (PreToolUse)** — blocks `git add` (including `git add -f`) of the gitignored beads exports (`.beads/{issues,interactions,events}.jsonl`). Beads is a first-class tracker throughout the plugin (`workflow-repo-preflight.py` detection, the `land-pr` and `work` skills' tracker bindings), but nothing previously stopped an agent from re-tracking these auto-regenerated exports and reintroducing the working-tree churn that beads projects gitignore them to avoid. Only activates when the exports actually exist under `.beads/`, so it's a no-op for non-beads projects.
  - **`check-gh-identity` (PreToolUse)** — blocks write-permission `gh pr|issue|repo|release` commands when the authenticated `gh` account doesn't match the project's expected account. Deliberately opt-in via `AGENTIC_ENGINEERING_GH_USER` (a repo's owner/org login isn't a reliable default — most repos are org-owned and contributors don't share a login with the org), and blocks with the exact `gh auth switch` fix rather than switching accounts itself, since silently mutating the machine's global `gh` identity is a bigger side effect than one repo's command warrants. Fills a gap the repo's own `ensure-gh-default-repo.sh` / `block-upstream-pr.sh` hooks don't cover: those solve *wrong repo* for this repo's own fork, but ship only in this repo's `.claude/hooks/` (not the distributable plugin), and neither guards against *wrong account* for downstream projects.
  - **`prime-beads-context` (SessionStart / PreCompact)** — runs `bd prime` when a project uses beads, surfacing ready work and conventions at session start and restoring that context after compaction (mirroring agent-leverage's own `SessionStart`/`PreCompact` → `bd prime` hooks). No-ops when `bd` isn't on PATH or `.beads/` doesn't exist.
