...
- **`block-beads-jsonl-stage` PreToolUse hook** (`scripts/block-beads-jsonl-stage.py`). Registers via `plugin.json` `hooks.PreToolUse`. Blocks `git add` (including `git add -f`) of the gitignored beads exports (`.beads/{issues,interactions,events}.jsonl`) in any project that installs the plugin. `bd` (the beads issue tracker, one of the tracker backends `skills/setup` already detects) regenerates these files on every mutation; tracking them produces constant working-tree churn. The hook turns a cryptic ".gitignore" failure — or a silent, damaging force-add — into a clear block that points at the real sync path (`bd dolt push` / `bd dolt pull`). Adapted from agent-leverage, generalized to not assume any particular repo's history.
- **`check-node-version` PreToolUse hook** (`scripts/check-node-version.py`). Registers alongside the other `Bash`-matched hooks. Before a `pnpm`/`npm`/`yarn`/`npx`/`turbo` command runs, compares the active `node --version` against the project's declared requirement (`.nvmrc` or `package.json#engines.node`) and blocks with the exact fix (a `./run` wrapper if one exists, otherwise an `nvm`/`fnm` switch command, otherwise an nvm install command) when they mismatch. Mismatched Node runtimes tend to fail with confusing downstream errors rather than a clear version message; this catches it before the command runs. No-op for any project without a declared Node version. Adapted from agent-leverage.
