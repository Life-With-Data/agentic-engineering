...
- **Safety-hook documentation + regression tests (adopted from `agent-leverage`).** The plugin already ships and auto-wires two PreToolUse guards — `block-no-verify` (blocks `git commit`/`git push --no-verify`) and `prevent-main-commit` (blocks direct commits/pushes to `main`/`master`) — but they were undocumented for end users and untested, despite being regex guards whose whole value is precision (they must ignore prose, quoted commit messages, chained command segments, and branches merely *named* like `main`). Ported `agent-leverage`'s hook-hardening discipline: a new `scripts/HOOKS.md` documents every shipped hook (what it blocks, its false-positive safeguards, how to test it, how to override it per-project), the plugin README gains a **Safety Hooks** section + Components row, and two new suites — `tests/block_no_verify_test.py` and `tests/prevent_main_commit_test.py` (23 cases total) — pin the tricky true-positive / false-positive edges. Both run in CI via the existing `python3 -m unittest discover` step (36 hook tests total, all green). No behaviour change to the hooks themselves — this locks in their behaviour so the regex guards can't silently regress.
