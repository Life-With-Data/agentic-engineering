...
- **`block-beads-jsonl-stage` hook — protects the Beads tracker integration from the JSONL-churn footgun.** A new `PreToolUse`/`Bash` guard (`scripts/block-beads-jsonl-stage.py`, wired in `plugin.json` alongside `block-no-verify` and `prevent-main-commit`) blocks any `git add` naming a Beads passive export (`.beads/{issues,interactions,events}.jsonl`) — including a force-add (`git add -f`) that would bypass `.gitignore`. `bd` regenerates these exports on every `bd create`/`update`/`close`, so tracking them produces constant working-tree churn; bead state is meant to be shared via `bd dolt push`, not git. The plugin already treats Beads as a first-class tracker (`/workflows:plan` writes `bead_id:`, `/workflows:work` runs `bd ready`/`bd close`, `/workflows:review` files findings as beads, and `workflow-repo-preflight.py` auto-detects it), but shipped no protection against re-tracking the exports; this closes that gap. The guard is precise — it sanitizes quoted strings and comments so prose mentioning the paths can't false-trigger, and ignores `git add -A`/`git add .` and tracked config like `.beads/config.yaml` — and is inert when Beads isn't in use. Adapted from a guardrail proven in the agent-leverage repo. Covered by `tests/block_beads_jsonl_stage_test.py`.
