...
- **`verification-loop` skill — a systematic verify-before-done loop (24th skill).** Adds a repeatable, language-agnostic verification pass — build → types → lint → tests → security sweep → diff review — that ends in a single report with a `READY` / `NOT READY` verdict, plus a checkpoint mode for long sessions. It fills the gap between per-tool hooks (immediate, narrow) and the review workflow (PR-time, broad): a self-contained loop an agent runs before declaring a change done. **First adoption through the `/upstream-scan` pipeline**, adapted from [affaan-m/ECC](https://github.com/affaan-m/ECC) (`skills/verification-loop/SKILL.md@81af4076`) per the `docs/upstream-sources.md` triage contract — rewritten into house conventions (what+when description, imperative voice), made stack-agnostic (detect the project's own build/lint/test commands rather than assuming npm/pnpm), and stripped of ECC-specific coupling (the `metadata.origin` frontmatter, the ECC `/verify` command reference, and the ECC hooks-subsystem note). Supply-chain review: single file, no scripts, no network/exfiltration/permission-weakening — clean. Registry entry moved from `deferred:` (shortlisted) to `adopted:`; from the 2026-07-02 upstream-adoption plan (issue #28).
