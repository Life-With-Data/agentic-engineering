...
- **`check-node-version` PreToolUse hook** (`scripts/check-node-version.py`). Registers alongside `block-no-verify` and `prevent-main-commit`. Before letting a package-manager command (`npm`/`pnpm`/`yarn`/`npx`/`turbo`) run, compares the active `node --version` against the project's declared requirement (`.nvmrc` or `package.json`'s `engines.node`) and blocks with the exact switch command (`nvm`/`fnm`/`volta`) on a mismatch. Node-version drift produces cryptic failures (native module ABI mismatches, ESM/CJS interop breaks) that read as code bugs but are actually environment drift — catching it before the command runs saves a debugging detour. No-ops entirely for non-Node projects or projects that don't declare a required version. Adapted from agent-leverage's operational toolchain (`check-node-version.py`), generalized to drop the project-specific `./run` wrapper assumption in favor of pure nvm/fnm/volta guidance. Covered by `tests/check_node_version_test.py` (9 cases via direct import + monkeypatched `node` lookup, so the suite doesn't depend on a real `node` binary being on the test runner's `PATH`).
