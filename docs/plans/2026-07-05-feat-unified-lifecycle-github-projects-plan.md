...
- [ ] **Prerequisite (blocker):** upgrade dev `gh` to ≥ 2.94.0; add explicit gh install/pin to `.github/workflows/ci.yml`. Record fixtures only from a ≥ 2.94.0 binary.
- [ ] New `scripts/lifecycle_board.py` per Architecture: pure decision core + injected `run_gh`/clock; CLI verbs `--gate/--claim/--set-status/--ready-work/--reconcile`; error contract `{ok, error_code, error, fix}`; in-script `--repo`/`--owner` discipline (security invariant 7); dataclasses serialized via `asdict`; extracted shared frontmatter parser
- [ ] `workflow-repo-preflight.py`: resolution enum (`github-project`/`github`/`none`), committed-config resolution via git-common-dir (worktree-safe), hyphen-tolerant config regex + `VALID_TRACKERS`, owner-equals-origin validation, new hard errors (`issues_disabled`, `unsupported_host`, `insufficient_permission` via `viewerCanUpdate`/`viewerPermission`), composes lifecycle_board read-only (repairs only under `--reconcile`); keep `beads_remember_available` (compound's `bd remember` survives — knowledge memory, not tracker state); tracker banner survives with new enum values (ambiguity suffix dies)
- [ ] Repo scoping + flags: every item read filters `content.repository.nameWithOwner == origin`; join-key normalization to `owner/repo#N`; reconciler `flags` output channel (`merged_to_non_default_branch`, `stale_join_key`, `truncated_ready_work`) with one-line issue comments; `--gate` emits `author_association`/`provenance`
- [ ] Session TTL cache (gitignored file): `last_reconciled_at` + per-session field/option IDs; batched-GraphQL helper shared by reconciler and ready-work; 403/429 jittered single retry; **verification spike: `gh project item-list --query`** (fallback: raw-GraphQL minimal-fields, max 2 pages)
- [ ] `plan-tracker-guard.py`: `TRACKER_FIELDS` → `("github_issue",)`; regex shrinks to `\d+ | org/repo#\d+`; remediation text gh-only; keep `issue_tracker: none` carve-out; **scope stays `docs/plans/` only** (brainstorm extension dropped — multi-turn deadlock)
- [ ] Extend `.claude/hooks/block-upstream-pr.sh`: `gh project` writes require `--owner` matching origin owner; match `GH_REPO=` env-prefix and `gh api` REST writes to non-origin `repos/...` paths; deny `gh api graphql` containing `ProjectV2` mutation names unless owner-validated. Plus the durable control: a **committed test** greping command/skill files for flagless `gh issue|project|api` invocations
- [ ] **Tests** (all named `*_test.py` for CI discovery — see Test strategy): rewritten `plan_tracker_guard_test.py`; `lifecycle_board_test.py` (tier 1 — gate/claim/repair/ready decision tables, closed-repair-set assertion, call-count budgets: ready-work ≤ 2 calls at 1000-item fixture, zero reconciler calls on warm TTL; **negative/topology cases**: mixed-repo board fixture (foreign items dropped, never written), repair 3 keys on `merged == false` not `state == closed` (merge queues), non-assignee closed PR fires nothing, qualified `owner/repo#N` join key, 50-item truncation flag); `gh_contract_test.py` (tier 2); `block_upstream_pr_test.py` (hook allow/deny matrix); `tests/fixtures/gh/` recorded fixtures + `record_gh_fixtures.sh` (hand-written mock JSON prohibited; fixtures carry the recording gh version)
