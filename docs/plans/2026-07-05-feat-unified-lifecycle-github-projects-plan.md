...
- [x] Setup skill records mode + writes the committed config (session cache needs no gitignore — it lives in `--git-common-dir`)

### Phase 5: Docs, diagrams, consistency, versioning

- [x] `FLOWS.md`: redraw around the lifecycle; add a mermaid `stateDiagram` (including the five repairs and terminal refinements) as the canonical visual
- [x] Plugin README: lifecycle table, board setup, human claim path ("assign yourself + drag to in_progress — the drag is the claim"), card-order-is-decorative note, beads-scratchpad note; `bun run docs:build`; CHANGELOG
- [x] **README "What this plugin assumes about your repo"** — the eyes-wide-open contract (D-class register rows): one board per repo; default-branch merges drive `shipped` (git-flow → the closer workflow, or cards stall with a reconciler comment); issues enabled; issue text is untrusted data; board is agent-managed after bootstrap (fresh-project guard, don't rename options); github.com only, gh ≥ 2.94.0 + `project` scope, project-write + triage permission; GitHub Free suffices for single-repo (its one auto-add workflow is all we use); macOS/Linux/WSL (native Windows untested); fork-based contribution needs the owner-allowlist entry
- [x] **README "Delivery-topology assumptions"** — per-topology expectations (from the state-flow review): trunk-based CD (deployed nearly redundant — fine to ignore), multi-environment (`deployed` = production only; staging never stamps), git-flow (stall + fix), release trains/libraries (`shipped` = "merged to default branch," NOT "in users' hands"; optional release-tag stamping), external CD (deployment_status adapter where Deployment records exist), no-deploy repos (`shipped → compounded` is the intended path); `deployed` is a high-water mark — rollbacks never move the board backward
- [x] **Counts after PR 2: 28 commands / 23 skills** (new `lifecycle-doctor` command; new `lifecycle` skill offsets the deleted `linear-sync`) — update all description strings + tables again
- [x] **README "Verify your setup"** section: run `/lifecycle-doctor` after install/bootstrap and before the first work item; re-run after changing board config, tokens, or CD wiring; `--live` for the full end-to-end probe
- [x] **Versioning policy:** both PRs share one CHANGELOG `[Unreleased] 3.0.0` entry; PR 1 sets the version fields; PR 2 amends the same entry (no second major bump); **no marketplace release/tag between the two merges**
- [x] Migration: **grandfather** — the in-flight plan (`github_issue: 28`) already carries the join key; legacy `status:`/`bead_id:` fields ignored and removed on next command touch; no one-shot script
...
### Functional
- [x] Board (Status 9 options + Priority) is the source of truth; every transition performed by its designated writer **or the shared reconciler applying one of the five documented repairs**
- [x] All workflow commands + triage/upstream-scan gate on entry via `--gate` and route around completed stages (runbook R1–R2)
- [x] Claim protocol survives concurrent claims (tier 1 decision test + runbook R3); blocked items never claimed; yield decisions assignee-anchored
- [x] Reconciler repair set is **closed at five** (tier-1 negative cases incl. merged==false, non-assignee PR, foreign-repo items) and every repair posts an issue comment; the three report-only flags emit comments + JSON, never writes
- [x] All board reads are repo-scoped (`content.repository.nameWithOwner == origin`); a mixed-repo board fixture shows foreign items filtered and never written
- [x] Bootstrap refuses non-fresh projects with a printed diff; derives owner from origin; disables "Item reopened"; run-twice idempotency keeps 9 options + workflows intact
- [x] Ready-work: planned ∧ unassigned ∧ unblocked, Priority-sorted, ≤ 2 API calls at any board size; hard-errors rather than returning empty on failure; truncation flagged at the 50-cap
- [x] Command-entry overhead ≤ ~2s / ≤ 5 `gh` calls steady-state; warm-TTL entries skip reconciliation
- [x] Security invariants present in the lifecycle skill + rewritten commands (untrusted-content rules, authorAssociation gate, no comment reads)
- [x] Linear: zero references outside CHANGELOG/git history; beads: no gate reads a bead; `bd remember` still works; `github` plain mode + `none` degrade as today

### Quality gates
- [x] `bun test` green (counts 27/22 after PR 1, **28/23 after PR 2**; version parity 3.0.0; docs:check; new flagless-gh grep test)
- [x] `/lifecycle-doctor` reports all-PASS on this repo (with `--live` evidence) before the runbook executes — the doctor is the runbook's step 0
- [x] `python3 -m unittest` green: tier 1 + tier 2 (help-text leg mandatory in CI at pinned gh ≥ 2.94.0; token-probe leg auto-skips unauthenticated) + hook tests
- [x] Nightly tier-3 smoke workflow committed (`.github/workflows/lifecycle-smoke.yml` — contract leg on GITHUB_TOKEN; board-probe leg gated on a `LIFECYCLE_BOARD_PAT` secret, skipped loudly without it). "Green once" satisfied by the **local live evidence** pre-merge: bootstrap ran against the real repo (board #5 created, ID-preserving option rewrite verified, probe PASS — issue #43 closed → automation stamped `shipped`) and `--doctor` reports 13/13 PASS; the scheduled run exercises the same legs post-merge (workflow_dispatch requires the workflow on the default branch)
- [x] Runbook R1–R4 executed on the live board (2026-07-06):
  - **R1 PASS** — full pipeline on a real fix (issue #46, work-gate reason text): brainstorm gate `proceed` → doc → `brainstormed`; plan → doc → `planned`; work gate `proceed` → `--claim` → `in_progress`; PR #47 with `Closes #46` → `in_review`; merge → **"Item closed" automation stamped `shipped` in <10s (observed)**; compound gate `proceed` → 3 solution docs → `compounded`. Every gate observed.
  - **R2 PASS** — brainstorm re-entry → `already_done`/`route_to_plan` (doc discovered by join key); plan re-entry → `already_done`/`route_to_work`. No duplicate issues or docs.
  - **R3 PASS (scoped)** — parallel same-user claims on a scratch issue: both idempotently succeed, single assignee, no board corruption. A true two-identity race is impossible on a single-collaborator repo; the conflict/yield decision paths are pinned by tier-1 (`decide_claim` multi-assignee) and the FakeRunner `verb_claim` e2e (visible `--remove-assignee` yield).
  - **R4 PASS** — human forward-drag `in_progress → in_review` mid-work: reconciler applied zero repairs (never fights a drag), the next gate read the dragged stage faithfully, drag-back clean. Bonus: the drag test surfaced a second stale-prose instance (the proceed reason), fixed in #47.
- [x] FLOWS.md renders; docs regenerated; CHANGELOG documents breaking changes + migration
